...
void RenderHyperboloidWithOneSheetGenerators(double a, double b, double c) {
	...
	for (double lambda = -3; lambda < 3; lambda += lambda_step) {
		for (double u = -5; u < 5; u += u_step) {
			Vertex3 v1, v2;
			v1.x = u;
			v1.y = (2 * v1.x / a - lambda - 1 / lambda) * b
					/ (lambda - 1 / lambda);
			v1.z = ((1 + v1.y / b) * lambda - v1.x / a) * c;

			v2.x = u + u_step;
			v2.y = (2 * v2.x / a - lambda - 1 / lambda) * b
					/ (lambda - 1 / lambda);
...
void RenderElipsoid(double a, double b, double c) {
	const double pi = 3.14159265;
	double u_step = pi / 100;
	double v_step = pi / 100;
	for (double u = 0; u <= pi; u += u_step) {
		for (double v = 0; v <= pi * 2; v += v_step) {
...
void RenderHyperboloidParaboloid(double a, double b, double c) {
	double u_step = 0.1;
	double v_step = 0.1;
	for (double u = -3; u < 3; u += u_step) {
		for (double v = -3; v <= 3; v += v_step) {
			...
			u3.y = v + v_step;
			u3.z = c
					* ((u + u_step) * (u + u_step) / a * a
							- (v + v_step) * (v + v_step) / b * b);
			u4.x = u + u_step;
			u4.y = v;
...
void RenderFunction() {
	...
	gluLookAt(camera_x, camera_y, camera_z, looking_x, looking_y, looking_z, 0,
			1, 0);
